...
<script type="text/javascript">
	var ckBaseUrl = "<%=com.thinkgem.jeesite.common.config.Global.getCkBaseUrl()%>";
	...
	function ${input}Preview(){
		...
		for (var i=0; i<urls.length; i++){
			if (urls[i]!=""){//<c:if test="${type eq 'thumb' || type eq 'images'}">
				...
				li = "<li>"+decodeURIComponent(urls[i].substring(urls[i].lastIndexOf("/")+1))+"";//</c:if>
				li += "&nbsp;&nbsp;</li>";
				$("#${input}Preview").append(li);
			}
		}
		if ($("#${input}Preview").text() == ""){
			$("#${input}Preview").html("<li style='list-style:none;padding-top:5px;'>None</li>");
		}
	}
	${input}Preview();
</script>