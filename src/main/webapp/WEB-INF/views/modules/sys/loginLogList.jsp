...
<head>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $("#btnExport").click(function(){
                ...
                top.$.jBox.confirm("确认要导出统计数据吗？","系统提示",function(v,h,f){
                    if(v == "ok"){
                        $("#searchForm").attr("action","${ctx}/report/exportSelectedLoginLogData").submit();
                    }
                },{buttonsFocus:1});
        ...
        function page(n,s){
            $("#pageNo").val(n);
            $("#pageSize").val(s);
            $("#searchForm").attr("action","${ctx}/report/queryLoginLogList").submit();
            return false;
        }
...
<body>
    <ul class="nav nav-tabs">
        <li class="active"><a href="${ctx}/log/queryLoginLogList">登录统计</a></li>
    </ul>
    ...
    <form:form id="searchForm" modelAttribute="loginLog" action="${ctx}/log/queryLoginLogList" method="post" class="breadcrumb form-search">
        ...
        <div>
            <label>归属公司：</label><sys:treeselect id="company" name="companyId" value="${loginLog.companyId}" labelName="companyName" labelValue="${loginLog.companyName}"
        ...
        <br/>
        <div>
            <label>归属部门：</label><sys:treeselect id="office" name="officeId" value="${loginLog.officeId}" labelName="officeName" labelValue="${loginLog.officeName}"
            ...
            <input id="btnExport" class="btn btn-primary" type="button" value="导出所选" />
    ...
    <table id="contentTable" class="table table-striped table-bordered table-condensed">
        ...
        <tbody>
        <c:forEach items="${page.list}" var="userOperation">
            <tr>
                <td>
                    <input type="checkbox" class="singleCheck" name="${userOperation.loginName}" value="${userOperation.officeId}">
                </td>
                <td>${userOperation.companyName}</td>
                <td>${userOperation.officeName}</td>
                <td>${userOperation.loginName}</td>
                <td>${userOperation.name}</td>
                <td>${userOperation.gender}</td>
                <td>${userOperation.count}</td>
            </tr>
        </c:forEach>
        </tbody>
    </table>
